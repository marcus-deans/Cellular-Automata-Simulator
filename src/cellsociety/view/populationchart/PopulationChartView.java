...
public class PopulationChartView extends Application {
  ...
  private static final ResourceBundle populationChartResources = ResourceBundle.getBundle(
      RESOURCE_FILE_PATH);
  ...
  public void start(Stage stage) {
    ...
    myPopulationChartScene.getStylesheets().add(
        PopulationChartView.class.getResource("PopulationChartViewFormatting.css")
            .toExternalForm());
    ...
    for (int counter = 0; counter < keyNumber; counter++) {
      populationChartData.add(
          new PieChart.Data(String.valueOf(counter), myStateCount.get(counter)));
    }
  ...
  private String getString(String key) {
    ...
    try {
      ...
      sendAlert(
          String.format("%s string was not found in Resource File %s", key, RESOURCE_FILE_PATH));
    }
    return value;
  }
